class §\x19\x04§.battlefield.§\x1a\x05\x14§.§\x1b\x18\x14§
{
   function §\x1b\x18\x14§()
   {
   }
   static function §\x1b\x18\x15§(§\x18\x1a§, §\x1d\x04\b§, §\x1c\x18\x0f§, §\x1c\x18\x0e§, §\x1b\x1b\x1c§)
   {
      var _loc7_ = _loc2_["\x1e\x18\x05"].Game["\x1d\x0f\t"];
      if(_loc4_ == undefined)
      {
         return null;
      }
      if(_loc5_ == undefined)
      {
         return null;
      }
      if(_loc7_ == undefined)
      {
         _loc7_ = false;
      }
      var _loc8_ = _loc6_.bAllDirections != undefined ? _loc6_.bAllDirections : true;
      var _loc9_ = _loc6_.nMaxLength != undefined ? _loc6_.nMaxLength : 500;
      var _loc10_ = _loc6_.bIgnoreSprites != undefined ? _loc6_.bIgnoreSprites : false;
      var _loc11_ = _loc6_.bCellNumOnly != undefined ? _loc6_.bCellNumOnly : false;
      var _loc12_ = _loc6_.bWithBeginCellNum != undefined ? _loc6_.bWithBeginCellNum : false;
      var _loc13_ = _loc3_["\x1d\x1a\x11"]();
      if(_loc8_)
      {
         var _loc14_ = 8;
         var _loc15_ = [1,_loc13_,_loc13_ * 2 - 1,_loc13_ - 1,-1,- _loc13_,- _loc13_ * 2 + 1,- (_loc13_ - 1)];
         var _loc16_ = [1.5,1,1.5,1,1.5,1,1.5,1];
      }
      else
      {
         _loc14_ = 4;
         _loc15_ = [_loc13_,_loc13_ - 1,- _loc13_,- (_loc13_ - 1)];
         _loc16_ = [1,1,1,1];
      }
      var _loc17_ = _loc3_["\x1e\x07\x1c"]();
      var _loc18_ = new Object();
      var _loc19_ = new Object();
      var _loc20_ = false;
      var _loc0_ = null;
      var _loc21_ = _loc18_["oCell" + _loc4_] = new Object();
      _loc21_.num = _loc4_;
      _loc21_.g = 0;
      _loc21_.v = 0;
      _loc21_.h = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1d\x19\x11"](_loc3_,_loc4_,_loc5_);
      _loc21_.f = _loc21_.h;
      _loc21_.l = _loc17_[_loc4_]["\x1d\x19\x06"];
      _loc21_.m = _loc17_[_loc4_]["\x1c\x1c\x01"];
      _loc21_["\x1b\x19\n"] = null;
      var _loc22_ = new Array();
      var _loc23_ = 0;
      while(_loc23_ < _loc17_.length - 1)
      {
         _loc22_[_loc23_] = _loc17_[_loc23_]["\x1d\f\x15"];
         _loc23_ = _loc23_ + 1;
      }
      var _loc24_ = new Array();
      if(!_loc10_ && !_loc7_)
      {
         var _loc25_ = 0;
         while(_loc25_ < _loc17_.length - 1)
         {
            _loc24_[_loc25_] = _loc17_[_loc25_]["\x1d\f\x18"];
            _loc25_ = _loc25_ + 1;
         }
      }
      while(!_loc20_)
      {
         var _loc26_ = null;
         var _loc27_ = 500000;
         §§enumerate(_loc18_);
         var _loc0_ = null;
         while((_loc0_ = §§enumeration()) != null)
         {
            if(_loc18_[k].f < _loc27_)
            {
               _loc27_ = _loc18_[k].f;
               _loc26_ = k;
            }
         }
         var _loc28_ = _loc18_[_loc26_];
         delete _loc18_[_loc26_];
         if(_loc28_.num == _loc5_)
         {
            var _loc29_ = new Array();
            while(_loc28_.num != _loc4_)
            {
               if(_loc28_.m == 0)
               {
                  _loc29_ = new Array();
               }
               else if(_loc11_)
               {
                  _loc29_.splice(0,0,_loc28_.num);
               }
               else
               {
                  _loc29_.splice(0,0,{num:_loc28_.num,dir:eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\x06\t"](_loc3_,_loc28_["\x1b\x19\n"].num,_loc28_.num)});
               }
               _loc28_ = _loc28_["\x1b\x19\n"];
            }
            if(_loc12_)
            {
               if(_loc11_)
               {
                  _loc29_.splice(0,0,_loc4_);
               }
               else
               {
                  _loc29_.splice(0,0,{num:_loc4_,dir:eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\x06\t"](_loc3_,_loc28_["\x1b\x19\n"].num,_loc4_)});
               }
            }
            return _loc29_;
         }
         var _loc30_ = false;
         var _loc31_ = 0;
         for(; _loc31_ < _loc14_; _loc31_ = _loc31_ + 1)
         {
            var _loc32_ = _loc28_.num + _loc15_[_loc31_];
            if(Math.abs(_loc17_[_loc32_].x - _loc17_[_loc28_.num].x) <= 53)
            {
               var _loc33_ = _loc17_[_loc32_];
               _loc30_ = !(_loc32_ == _loc5_ && _loc33_["\x1c\x1c\x01"] == 1) ? false : true;
               var _loc34_ = _loc33_["\x1d\x19\x06"];
               var _loc35_ = _loc28_.l == undefined || Math.abs(_loc34_ - _loc28_.l) < 2;
               if(!_loc35_ || (!_loc33_["\x1e\x06"] || _loc33_["\x1c\x1c\x01"] == 1 && !_loc30_))
               {
                  continue;
               }
               var _loc36_ = true;
               if(!_loc10_)
               {
                  var _loc37_ = _loc33_["\x1a\x15\x0e"];
                  if(_loc7_)
                  {
                     _loc36_ = _loc37_ == undefined ? true : false;
                  }
                  else
                  {
                     var _loc38_ = _loc2_.gfx["\x1a\x15\x12"]["\x1d\x1b\x16"](_loc37_);
                     _loc36_ = !(_loc38_ != undefined && (_loc38_ instanceof dofus["\x1e\x18\x05"]["\x03\x1d"] && _loc32_ != _loc5_)) ? true : false;
                  }
                  if(_loc36_ && (_loc32_ != _loc5_ && _loc24_[_loc32_] == true))
                  {
                     _loc36_ = false;
                  }
               }
               if(_loc36_ && (_loc32_ != _loc5_ && _loc22_[_loc32_] == true))
               {
                  _loc36_ = false;
               }
               if(!_loc36_)
               {
                  continue;
               }
               var _loc39_ = "oCell" + _loc32_;
               var _loc40_ = _loc28_.v + _loc16_[_loc31_] + (!(_loc33_["\x1c\x1c\x01"] == 0 || _loc33_["\x1c\x1c\x01"] == 1) ? 0 : 1000 + (_loc31_ % 2 != 0 ? 0 : 3)) + (!(_loc33_["\x1c\x1c\x01"] == 1 && _loc30_) ? (_loc31_ == _loc28_.d ? 0 : 0.5) + (5 - _loc33_["\x1c\x1c\x01"]) / 3 : -1000);
               var _loc41_ = _loc28_.g + _loc16_[_loc31_];
               var _loc42_ = null;
               if(_loc18_[_loc39_])
               {
                  _loc42_ = _loc18_[_loc39_].v;
               }
               else if(_loc19_[_loc39_])
               {
                  _loc42_ = _loc19_[_loc39_].v;
               }
               if((_loc42_ == null || _loc42_ > _loc40_) && _loc41_ <= _loc9_)
               {
                  if(_loc19_[_loc39_])
                  {
                     delete _loc19_[_loc39_];
                  }
                  var _loc43_ = new Object();
                  _loc43_.num = _loc32_;
                  _loc43_.g = _loc41_;
                  _loc43_.v = _loc40_;
                  _loc43_.h = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1d\x19\x11"](_loc3_,_loc32_,_loc5_);
                  _loc43_.f = _loc43_.v + _loc43_.h;
                  _loc43_.d = _loc31_;
                  _loc43_.l = _loc34_;
                  _loc43_.m = _loc33_["\x1c\x1c\x01"];
                  _loc43_["\x1b\x19\n"] = _loc28_;
                  _loc18_[_loc39_] = _loc43_;
               }
            }
         }
         _loc19_["oCell" + _loc28_.num] = {v:_loc28_.v};
         _loc20_ = true;
         for(_loc20_ in _loc18_)
         {
            break;
         }
      }
      return null;
   }
   static function §\x1d\x19\x11§(§\x1d\x04\b§, §\x1c\x18\x11§, §\x1c\x18\x10§)
   {
      var _loc5_ = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\b\x04"](_loc2_,_loc3_);
      var _loc6_ = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\b\x04"](_loc2_,_loc4_);
      var _loc7_ = Math.abs(_loc5_.x - _loc6_.x);
      var _loc8_ = Math.abs(_loc5_.y - _loc6_.y);
      return Math.sqrt(Math.pow(_loc7_,2) + Math.pow(_loc8_,2));
   }
   static function §\x1d\x19\x12§(§\x1d\x04\b§, §\x1c\x18\x11§, §\x1c\x18\x10§)
   {
      var _loc5_ = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\b\x04"](_loc2_,_loc3_);
      var _loc6_ = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\b\x04"](_loc2_,_loc4_);
      var _loc7_ = Math.abs(_loc5_.x - _loc6_.x);
      var _loc8_ = Math.abs(_loc5_.y - _loc6_.y);
      return _loc7_ + _loc8_;
   }
   static function §\x1e\b\x04§(§\x1d\x04\b§, §\x1c\x12\x03§)
   {
      var _loc4_ = _loc2_["\x1d\x1a\x11"]();
      var _loc5_ = Math.floor(_loc3_ / (_loc4_ * 2 - 1));
      var _loc6_ = _loc3_ - _loc5_ * (_loc4_ * 2 - 1);
      var _loc7_ = _loc6_ % _loc4_;
      var _loc8_ = new Object();
      _loc8_.y = _loc5_ - _loc7_;
      _loc8_.x = (_loc3_ - (_loc4_ - 1) * _loc8_.y) / _loc4_;
      return _loc8_;
   }
   static function §\x1e\x06\t§(§\x1d\x04\b§, §\x1c\x18\x11§, §\x1c\x18\x10§)
   {
      var _loc5_ = _loc2_["\x1d\x1a\x11"]();
      var _loc6_ = [1,_loc5_,_loc5_ * 2 - 1,_loc5_ - 1,-1,- _loc5_,- _loc5_ * 2 + 1,- (_loc5_ - 1)];
      var _loc7_ = _loc4_ - _loc3_;
      var _loc8_ = 7;
      while(_loc8_ >= 0)
      {
         if(_loc6_[_loc8_] == _loc7_)
         {
            return _loc8_;
         }
         _loc8_ = _loc8_ - 1;
      }
      var _loc9_ = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\b\x04"](_loc2_,_loc3_);
      var _loc10_ = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\b\x04"](_loc2_,_loc4_);
      var _loc11_ = _loc10_.x - _loc9_.x;
      var _loc12_ = _loc10_.y - _loc9_.y;
      if(_loc11_ == 0)
      {
         if(_loc12_ > 0)
         {
            return 3;
         }
         return 7;
      }
      if(_loc11_ > 0)
      {
         return 1;
      }
      return 5;
   }
   static function §\x1e\x06\b§(§\x1a\x03\x05§, §\x1a\x02\x05§, §\x1a\x03\x04§, §\x1a\x02\x04§, §\x14\n§)
   {
      var _loc7_ = Math.atan2(_loc5_ - _loc3_,_loc4_ - _loc2_);
      if(_loc6_)
      {
         if(_loc7_ >= (- Math.PI) / 8 && _loc7_ < Math.PI / 8)
         {
            return 0;
         }
         if(_loc7_ >= Math.PI / 8 && _loc7_ < Math.PI / 3)
         {
            return 1;
         }
         if(_loc7_ >= Math.PI / 3 && _loc7_ < 2 * Math.PI / 3)
         {
            return 2;
         }
         if(_loc7_ >= 2 * Math.PI / 3 && _loc7_ < 7 * Math.PI / 8)
         {
            return 3;
         }
         if(_loc7_ >= 7 * Math.PI / 8 || _loc7_ < -7 * Math.PI / 8)
         {
            return 4;
         }
         if(_loc7_ >= -7 * Math.PI / 8 && _loc7_ < -2 * Math.PI / 3)
         {
            return 5;
         }
         if(_loc7_ >= -2 * Math.PI / 3 && _loc7_ < (- Math.PI) / 3)
         {
            return 6;
         }
         if(_loc7_ >= (- Math.PI) / 3 && _loc7_ < (- Math.PI) / 8)
         {
            return 7;
         }
      }
      else
      {
         if(_loc7_ >= 0 && _loc7_ < Math.PI / 2)
         {
            return 1;
         }
         if(_loc7_ >= Math.PI / 2 && _loc7_ <= Math.PI)
         {
            return 3;
         }
         if(_loc7_ >= - Math.PI && _loc7_ < (- Math.PI) / 2)
         {
            return 5;
         }
         if(_loc7_ >= (- Math.PI) / 2 && _loc7_ < 0)
         {
            return 7;
         }
      }
      return 1;
   }
   static function §\x1e\b\x11§(§\x1d\x04\b§, §\x1c\x18\f§, §\x1c\x16\x1b§, §\x1c\x14\n§)
   {
      var _loc6_ = _loc2_["\x1d\x1a\x11"]();
      var _loc7_ = [1,_loc6_,_loc6_ * 2 - 1,_loc6_ - 1,-1,- _loc6_,- _loc6_ * 2 + 1,- (_loc6_ - 1)];
      var _loc8_ = 0;
      switch(_loc5_ % 8)
      {
         case 0:
            _loc8_ = 2;
            break;
         case 1:
            _loc8_ = 6;
            break;
         case 2:
            _loc8_ = 4;
            break;
         case 3:
            _loc8_ = 0;
            break;
         case 4:
            _loc8_ = 3;
            break;
         case 5:
            _loc8_ = 5;
            break;
         case 6:
            _loc8_ = 1;
            break;
         case 7:
            _loc8_ = 7;
      }
      _loc8_ = (_loc8_ + _loc4_) % 8;
      var _loc9_ = _loc3_ + _loc7_[_loc8_];
      var _loc10_ = _loc2_["\x1e\x07\x1c"]();
      var _loc11_ = _loc10_[_loc9_];
      if(_loc11_["\x1e\x06"] && (_loc10_[_loc9_] != undefined && Math.abs(_loc10_[_loc9_].x - _loc10_[_loc3_].x) <= 53))
      {
         return _loc9_;
      }
      return _loc3_;
   }
   static function §\x1e\x1b\x18§(§\x1c\x16\x1c§)
   {
      return _loc2_ | 1;
   }
   static function §\x1d\x1c\b§(§\x1a\x19\x12§, §\x1d\b\x16§, §\x1a\x19\x11§, §\x1d\b\x15§, §\x1e\x15\f§)
   {
      switch(_loc6_)
      {
         case 0:
            if(((_loc2_ - 1 & 2) >> 1) + _loc3_ != (_loc4_ - 1 & 1) + _loc5_)
            {
               return false;
            }
            break;
         case 1:
            if(((_loc2_ - 1 & 4) >> 2) + _loc3_ != ((_loc4_ - 1 & 2) >> 1) + _loc5_)
            {
               return false;
            }
            if(((_loc2_ - 1 & 8) >> 3) + _loc3_ != (_loc4_ - 1 & 1) + _loc5_)
            {
               return false;
            }
            break;
         case 2:
            if(((_loc2_ - 1 & 8) >> 3) + _loc3_ != ((_loc4_ - 1 & 2) >> 1) + _loc5_)
            {
               return false;
            }
            break;
         case 3:
            if(((_loc2_ - 1 & 8) >> 3) + _loc3_ != ((_loc4_ - 1 & 4) >> 2) + _loc5_)
            {
               return false;
            }
            if((_loc2_ - 1 & 1) + _loc3_ != ((_loc4_ - 1 & 2) >> 1) + _loc5_)
            {
               return false;
            }
            break;
         case 4:
            if((_loc2_ - 1 & 1) + _loc3_ != ((_loc4_ - 1 & 4) >> 2) + _loc5_)
            {
               return false;
            }
            break;
         case 5:
            if((_loc2_ - 1 & 1) + _loc3_ != ((_loc4_ - 1 & 8) >> 3) + _loc5_)
            {
               return false;
            }
            if(((_loc2_ - 1 & 2) >> 1) + _loc3_ != ((_loc4_ - 1 & 4) >> 2) + _loc5_)
            {
               return false;
            }
            break;
         case 6:
            if(((_loc2_ - 1 & 2) >> 1) + _loc3_ != ((_loc4_ - 1 & 8) >> 3) + _loc5_)
            {
               return false;
            }
            break;
         case 7:
            if(((_loc2_ - 1 & 2) >> 1) + _loc3_ != (_loc4_ - 1 & 1) + _loc5_)
            {
               return false;
            }
            if(((_loc2_ - 1 & 4) >> 2) + _loc3_ != ((_loc4_ - 1 & 8) >> 3) + _loc5_)
            {
               return false;
            }
            break;
      }
      return true;
   }
   static function §\x02\x02§(§\x1d\x04\b§, §\x05\x0b§, §\x05\n§)
   {
      var _loc5_ = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\b\x04"](_loc2_,_loc3_);
      var _loc6_ = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\b\x04"](_loc2_,_loc4_);
      var _loc7_ = _loc2_["\x1e\b\x01"](_loc3_);
      var _loc8_ = _loc2_["\x1e\b\x01"](_loc4_);
      var _loc9_ = !_loc7_["\x1a\x15\x0e"] ? 0 : 1.5;
      var _loc10_ = !_loc8_["\x1a\x15\x0e"] ? 0 : 1.5;
      _loc9_ += !_loc7_["\x05\x19"] ? 0 : 1.5;
      _loc10_ += !_loc8_["\x05\x19"] ? 0 : 1.5;
      _loc5_.z = _loc2_["\x1e\x07\x1d"](_loc3_) + _loc9_;
      _loc6_.z = _loc2_["\x1e\x07\x1d"](_loc4_) + _loc10_;
      var _loc11_ = _loc6_.z - _loc5_.z;
      var _loc12_ = Math.max(Math.abs(_loc5_.y - _loc6_.y),Math.abs(_loc5_.x - _loc6_.x));
      var _loc13_ = (_loc5_.y - _loc6_.y) / (_loc5_.x - _loc6_.x);
      var _loc14_ = _loc5_.y - _loc13_ * _loc5_.x;
      var _loc15_ = _loc6_.x - _loc5_.x >= 0 ? 1 : -1;
      var _loc16_ = _loc6_.y - _loc5_.y >= 0 ? 1 : -1;
      var _loc17_ = _loc5_.y;
      var _loc18_ = _loc5_.x;
      var _loc19_ = _loc6_.x * _loc15_;
      var _loc20_ = _loc6_.y * _loc16_;
      var _loc27_ = _loc5_.x + 0.5 * _loc15_;
      while(_loc27_ * _loc15_ <= _loc19_)
      {
         var _loc25_ = _loc13_ * _loc27_ + _loc14_;
         if(_loc16_ > 0)
         {
            var _loc21_ = Math.round(_loc25_);
            var _loc22_ = Math.ceil(_loc25_ - 0.5);
         }
         else
         {
            _loc21_ = Math.ceil(_loc25_ - 0.5);
            _loc22_ = Math.round(_loc25_);
         }
         var _loc26_ = _loc17_;
         while(_loc26_ * _loc16_ <= _loc22_ * _loc16_)
         {
            if(!eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x02\x1c"](_loc2_,_loc27_ - _loc15_ / 2,_loc26_,false,_loc5_,_loc6_,_loc11_,_loc12_))
            {
               return false;
            }
            _loc26_ += _loc16_;
         }
         _loc17_ = _loc21_;
         _loc27_ += _loc15_;
      }
      _loc26_ = _loc17_;
      while(_loc26_ * _loc16_ <= _loc6_.y * _loc16_)
      {
         if(!eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x02\x1c"](_loc2_,_loc27_ - 0.5 * _loc15_,_loc26_,false,_loc5_,_loc6_,_loc11_,_loc12_))
         {
            return false;
         }
         _loc26_ += _loc16_;
      }
      if(!eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x02\x1c"](_loc2_,_loc27_ - 0.5 * _loc15_,_loc26_ - _loc16_,true,_loc5_,_loc6_,_loc11_,_loc12_))
      {
         return false;
      }
      return true;
   }
   static function §\x02\x1c§(§\x1d\x04\b§, §\x1a\x03\x06§, §\x1a\x02\x06§, §\r\x01§, §\x1b\x19\x15§, §\x1b\x19\x14§, §\x1a\x01\x17§, §\x1e\x18\x11§)
   {
      var _loc10_ = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\b\x03"](_loc2_,_loc3_,_loc4_);
      var _loc11_ = _loc2_["\x1e\b\x01"](_loc10_);
      var _loc12_ = Math.max(Math.abs(_loc6_.y - _loc4_),Math.abs(_loc6_.x - _loc3_));
      var _loc13_ = _loc12_ / _loc9_ * _loc8_ + _loc6_.z;
      var _loc14_ = _loc2_["\x1e\x07\x1d"](_loc10_);
      var _loc15_ = !(_loc11_["\x1a\x15\x0e"] == undefined || (_loc12_ == 0 || (_loc5_ || _loc7_.x == _loc3_ && _loc7_.y == _loc4_))) ? true : false;
      if(_loc11_["\x1d\x07\x19"] && (_loc11_["\x1e\x06"] && (_loc14_ <= _loc13_ && !_loc15_)))
      {
         return true;
      }
      if(_loc5_)
      {
         return true;
      }
      return false;
   }
   static function §\x1e\b\x03§(§\x1d\x04\b§, §\x1a\x03\x06§, §\x1a\x02\x06§)
   {
      var _loc5_ = _loc2_["\x1d\x1a\x11"]();
      return _loc3_ * _loc5_ + _loc4_ * (_loc5_ - 1);
   }
   static function §\x03\t§(§\x1d\x04\b§, §\x05\x0b§, §\x05\n§)
   {
      var _loc5_ = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\b\x04"](_loc2_,_loc3_);
      var _loc6_ = eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1e\b\x04"](_loc2_,_loc4_);
      if(_loc5_.x == _loc6_.x)
      {
         return true;
      }
      if(_loc5_.y == _loc6_.y)
      {
         return true;
      }
      return false;
   }
   static function §\x02\x07§(§\x1d\x04\b§, §\x1c\x18\x11§, §\x1c\x18\x10§, §\x0e\x15§, §\x1c\x0f\x19§, §\x1c\x0f\x1a§, §\x1c\x0f\x18§)
   {
      _loc6_ = Number(_loc6_);
      _loc7_ = Number(_loc7_);
      _loc8_ = Number(_loc8_);
      if(_loc7_ != 0)
      {
         _loc7_ += _loc8_;
         _loc7_ = Math.max(_loc6_,_loc7_);
      }
      if(_loc5_)
      {
         if(!eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x03\t"](_loc2_,_loc3_,_loc4_))
         {
            return false;
         }
      }
      if(eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1d\x19\x12"](_loc2_,_loc3_,_loc4_) > _loc7_ || eval("\x19\x04").battlefield["\x1a\x05\x14"]["\x1b\x18\x14"]["\x1d\x19\x12"](_loc2_,_loc3_,_loc4_) < _loc6_)
      {
         return false;
      }
      return true;
   }
}
