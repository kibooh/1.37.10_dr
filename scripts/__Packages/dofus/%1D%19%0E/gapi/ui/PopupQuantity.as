class dofus.§\x1d\x19\x0e§.gapi.ui.PopupQuantity extends dofus.§\x1d\x19\x0e§.gapi.§\x1e\x1b\x10§.§\x1e\x13\x15§
{
   static var §\x01\x0f§ = "PopupQuantity";
   var §\x17\x1b\x15§ = 0;
   var §\x19\x11\x01§ = false;
   var §\x18\x03\x0f§ = 1;
   var §\x18\x02\x18§ = 1;
   var _bMaxButtonValidationEnabled = true;
   function PopupQuantity()
   {
      super();
   }
   function §\x15\x1b\x14§(§\x1c\f\x0e§)
   {
      this["\x17\x1b\x15"] = _loc2_;
      var _loc3_ = this._winBackground.content;
      _loc3_["\x17\x0e\r"].text = _loc2_;
      _loc3_["\x17\x0e\r"].setFocus();
      return this["\x16\x0f\x19"]();
   }
   function §\x16\x0f\x19§()
   {
      return this["\x17\x1b\x15"];
   }
   function §\x16\x05\x01§(§\x1c\x13\x05§)
   {
      if(_loc2_ == undefined || _loc2_ < this["\x18\x02\x18"])
      {
         this["\x18\x03\x0f"] = this["\x18\x02\x18"];
      }
      else
      {
         this["\x18\x03\x0f"] = _loc2_;
      }
      return this["\x16\x19\x12"]();
   }
   function §\x16\x04\x06§(§\x1c\x12\x10§)
   {
      this["\x18\x02\x18"] = _loc2_;
      return this["\x16\x18\x1b"]();
   }
   function §\x15\x1b\x18§(§\b\x18§)
   {
      this["\x19\x11\x01"] = _loc2_;
      return this.__get__useAllStage();
   }
   function get isMaxButtonValidationEnabled()
   {
      return this._bMaxButtonValidationEnabled;
   }
   function set isMaxButtonValidationEnabled(bMaxButtonValidationEnabled)
   {
      this._bMaxButtonValidationEnabled = bMaxButtonValidationEnabled;
   }
   function §\x1d\x13\x1a§()
   {
      super["\x1d\x13\x1a"](false,dofus["\x1d\x19\x0e"].gapi.ui.PopupQuantity["\x01\x0f"]);
   }
   function §\x1e\x1a\x14§()
   {
      this["\x1b\x1c"]({object:this,method:this["\x1d\x05"]});
   }
   function §\x07\x16§()
   {
      this["\x1a\t\x07"]();
      return true;
   }
   function §\x1d\x05§()
   {
      this._winBackground["\x1d\x15"]("complete",this);
      this._bgHidder["\x1d\x15"]("click",this);
      this.api["\x1d\x0b\x04"]["\x1d\x0b\x02"]["\x1c\x11"]("onShortcut",this);
   }
   function §\x1d\x12\b§()
   {
      var _loc2_ = this._winBackground.content;
      _loc2_._btnOk["\x1d\x15"]("click",this);
      _loc2_._btnMax["\x1d\x15"]("click",this);
      _loc2_._btnMin["\x1d\x15"]("click",this);
      _loc2_._btnMax.label = this.api.lang.getText("MAX_WORD");
      _loc2_._btnMin.label = this.api.lang.getText("MIN_WORD");
      _loc2_["\x17\x0e\r"].restrict = "0-9";
      _loc2_["\x17\x0e\r"].text = this["\x17\x1b\x15"];
      _loc2_["\x17\x0e\r"]["\x1d\x15"]("change",this);
      _loc2_["\x17\x0e\r"].setFocus();
   }
   function §\x1b\x17\x14§()
   {
      var _loc2_ = this._xmouse - this._winBackground.width;
      var _loc3_ = this._ymouse - this._winBackground._height;
      var _loc4_ = !this["\x19\x11\x01"] ? this.gapi["\x1b\x0b\x13"] : Stage.width;
      var _loc5_ = !this["\x19\x11\x01"] ? this.gapi["\x1b\x0b\x15"] : Stage.height;
      if(_loc2_ < 0)
      {
         _loc2_ = 0;
      }
      if(_loc3_ < 0)
      {
         _loc3_ = 0;
      }
      if(_loc2_ > _loc4_ - this._winBackground.width)
      {
         _loc2_ = _loc4_ - this._winBackground.width;
      }
      if(_loc3_ > _loc5_ - this._winBackground.height)
      {
         _loc3_ = _loc5_ - this._winBackground.height;
      }
      this._winBackground._x = _loc2_;
      this._winBackground._y = _loc3_;
   }
   function validate()
   {
      this.api["\x1d\x0b\x04"]["\x1d\x0b\x02"]["\x1b\x10\x05"](this);
      this["\x1e\x15\x06"]({type:"validate",value:this.value,params:this["\x17\x17\x19"]});
   }
   function change(§\x1c\n\x07§)
   {
      var _loc3_ = this._winBackground.content;
      var _loc4_ = Number(_loc3_["\x17\x0e\r"].text);
      if(_global.isNaN(_loc4_))
      {
         this.value = this["\x18\x02\x18"];
      }
      else if(_loc4_ > this["\x18\x03\x0f"])
      {
         this.value = this["\x18\x03\x0f"];
      }
      else if(_loc4_ < this["\x18\x02\x18"])
      {
         this.value = this["\x18\x02\x18"];
      }
      else
      {
         this["\x17\x1b\x15"] = _loc4_;
      }
   }
   function complete(§\x1c\n\x07§)
   {
      this["\x1b\x17\x14"]();
      this["\x1b\x1c"]({object:this,method:this["\x1d\x12\b"]});
   }
   function click(§\x1c\n\x07§)
   {
      switch(_loc2_.target._name)
      {
         case "_btnOk":
            this.validate();
            break;
         case "_btnMax":
            if(this._bMaxButtonValidationEnabled && this.value == this["\x18\x03\x0f"])
            {
               this.validate();
               break;
            }
            this.value = this["\x18\x03\x0f"];
            return undefined;
         case "_bgHidder":
         default:
            break;
         case "_btnMin":
            this.value = this["\x18\x02\x18"];
            return undefined;
      }
      this["\x1a\t\x07"]();
   }
   function onShortcut(§\x1a\x14\x10§)
   {
      if(_loc2_ == "ACCEPT_CURRENT_DIALOG")
      {
         this.validate();
         this["\x1a\t\x07"]();
         return false;
      }
      return true;
   }
}
